...
public class AdminDashboardController {
    ...
    @FXML
    private Label welcomeLabel, occupancyCount, overstayCount, ruleStatusLabel;
    @FXML
    private TextField ruleNameField, ruleStartField, ruleEndField, ruleMaxDuration, ruleMaxVisitors;
    @FXML
    private ComboBox<String> ruleCategoryCombo;

    // Rules table
    @FXML
    private TableView<AccessRule> rulesTable;
    @FXML
    private TableColumn<AccessRule, String> colRuleName, colRuleCat, colRuleTime, colRuleDuration, colRuleVisitors,
            colRuleActive;

    // Occupancy table
    @FXML
    private TableView<EntryLog> occupancyTable;
    @FXML
    private TableColumn<EntryLog, String> colOccName, colOccType, colOccCategory, colOccEntry, colOccDuration,
            colOccUnit;
    ...
    @FXML
    private TableColumn<Violation, String> colViolId, colViolType, colViolPerson, colViolDesc, colViolStatus,
            colViolAction, colViolDate;
    ...
    public void initialize() {
        ...
        if (session != null)
            welcomeLabel.setText("Admin: " + session.getFullName());

        ruleCategoryCombo
                .setItems(FXCollections.observableArrayList("Guest", "Delivery", "Service", "Contractor", "Other"));

        // Rules table columns
        colRuleName
                .setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getRuleName()));
        colRuleCat
                .setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getCategory()));
        colRuleTime.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getTimeWindow()));
        colRuleDuration.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getMaxDurationMinutes() + " min"));
        colRuleVisitors.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(
                String.valueOf(cd.getValue().getMaxVisitorsPerDay())));
        colRuleActive.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getActiveStatus()));

        // Occupancy table columns
        colOccName.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(
                cd.getValue().getPersonName() != null ? cd.getValue().getPersonName()
                        : "ID:" + cd.getValue().getPersonId()));
        colOccType.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getPersonType().name()));
        colOccCategory
                .setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getCategory()));
        colOccEntry.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(
                cd.getValue().getEntryTimestamp() != null
                        ? cd.getValue().getEntryTimestamp().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))
                        : ""));
        colOccDuration.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getDurationFormatted()));
        ...
        colViolId.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(String.valueOf(cd.getValue().getViolationId())));
        colViolType.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getViolationTypeString()));
        ...
        colViolDesc.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getDescription()));
        colViolStatus.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getStatusString()));
        colViolAction.setCellValueFactory(
                cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getActionString()));
        colViolDate.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(
                cd.getValue().getDetectedAt() != null
                        ? cd.getValue().getDetectedAt().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))
                        : ""));
    ...
    private void handleSaveRule() {
        try {
            ...
            if (name.isEmpty() || cat == null || startStr.isEmpty() || endStr.isEmpty() || durStr.isEmpty()
                    || visStr.isEmpty()) {
            ...
            if (selected != null) {
                ...
                ruleStatusLabel.setText("✅ Rule updated.");
                ruleStatusLabel.getStyleClass().setAll("success-label");
            } else {
    ...
    private void handleWarning() {
        takeViolationAction("WARNING");
    }

    @FXML
    private void handleBlacklist() {
        takeViolationAction("BLACKLIST");
    }

    @FXML
    private void handleResolve() {
        takeViolationAction("FINE");
    }
